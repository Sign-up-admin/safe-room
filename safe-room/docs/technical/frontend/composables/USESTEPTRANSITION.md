...
useStepTransition 组合式函数的功能描述和使用场景。
...
const { result, loading, error, execute } = useStepTransition()
```
...
interface UseStepTransitionReturn<T = any> {
  result: Ref<T | null>
  loading: Ref<boolean>
