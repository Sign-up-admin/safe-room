...
const { result, loading, error, execute } = useModuleCrud()
```
...
```typescript
interface UseModuleCrudOptions {
  param1?: string
}

interface UseModuleCrudReturn<T = any> {
  result: Ref<T | null>
  loading: Ref<boolean>
