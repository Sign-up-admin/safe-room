---
title: ADMIN LOGIN REQUIREMENTS
version: v1.0.0
last_updated: 2025-11-16
status: active
category: requirements
---# 后台登录页面需求文档（Admin Login v2.0）
...
| 功能 | 说明 | 接口/数据 | 边界场景 |
| --- | --- | --- | --- |
| 角色列表加载 | 根据 `menu.list()` 中 `hasBackLogin` 字段过滤 | 本地 `menu.ts` + 缓存 | 无可登录角色时提示“请联系管理员” |
| 表单校验 | 账号/密码必填，角色>1 时必选 | 前端校验 + `FormRules` | 点击回车触发提交 |
| 登录请求 | `POST /{tableName}/login`，参数 `username`、`password`（表单提交或 JSON） | `http.post` | 网络失败、密码错误、账户锁定 |
| Token 存储 | 成功后写入 `storage`：`Token`, `role`, `sessionTable`, `adminName` | `src/utils/storage.ts` | 支持“记住账号”选项（localStorage） |
| 错误反馈 | `ElMessage.error` 展示接口返回的 `msg` | - | 超过 5 次错误触发验证码/冷却提示 |
| 跳转逻辑 | 登录成功跳转 `/`，若路由有 `redirect` 参数优先使用 | Vue Router | Token 过期时重定向回登录并提示 |
| 错误页面联动 | 从 401/403 错误页跳转时，登录成功后返回原页面 | `router.query.redirect` | 需验证 redirect 路径安全性 |
| 多语言 | 表单标签、按钮需进入 i18n 资源 | `/src/utils/i18n.ts` | -
...
1. HTTPS 下传输账号密码；后端返回 Token，前端不存储密码。  
2. 支持简单防爆破策略：N 次失败后显示验证码/延迟接口。  
3. Token 失效时手动清理本地存储，防止直接返回上一页。  
4. 登录页禁止嵌入 iframe，添加 `X-Frame-Options: DENY`。  
5. 若浏览器兼容，记住账号时需提示“此操作仅限个人设备”。
...
| --- | --- |
| 功能 | 正确加载角色，支持多角色登录流程 |
| 性能 | 首屏渲染 ≤ 1s；接口成功后 300ms 内跳转 |
| 安全 | Token 正确存储，退出后清理；无明文密码 |
| 无障碍 | 支持键盘操作、ARIA 标签（`aria-label`） |
| 文案 | 中英双语，错误信息友好 |
...
- [错误响应与非法访问拦截页面需求文档](./requirements\requirements\ERROR_PAGES_REQUIREMENTS.md)
- [安全文档](./SECURITY.md)
