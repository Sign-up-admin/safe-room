---
title: ADMIN CONFIG CENTER REQUIREMENTS
version: v1.0.0
last_updated: 2025-11-16
status: active
category: requirements
---# 配置中心需求文档（Admin Config Center v2.0）

> 版本：v2.0
> 更新日期：2025-11-16（基于现有实现更新）
> 适用路由：`/config`, `/news`, `/newstype`, `/chat`
> 关联组件：`src/views/modules/config/list.vue` 等专用组件
...
基于`config/list.vue`的实际实现：

| 功能 | 说明 | 接口 | 状态 |
| --- | --- | --- | --- |
| 列表查询 | 支持分页、默认按添加时间倒序 | `GET /config/list` | ✅ 已实现 |
| 排序功能 | 数字输入框手动调整sort字段 | 实时保存 | ✅ 已实现 |
| 新增轮播 | 表单弹窗，包含图片上传和字段编辑 | `POST /config/save` | ✅ 已实现 |
| 编辑轮播 | 修改现有记录的所有字段 | `POST /config/update` | ✅ 已实现 |
| 删除轮播 | 支持单个删除，预留批量删除 | `POST /config/delete` | ✅ 已实现 |
| 图片预览 | 点击缩略图可预览大图 | Element Plus Image组件 | ✅ 已实现 |
| 查看详情 | JSON格式显示完整记录 | 弹窗显示 | ✅ 已实现 |

### 2.4 交互规范

基于实际实现的特点：

- **排序调整**：每行都有排序输入框，范围0-999，支持实时修改
- **图片展示**：缩略图100×60px，点击预览，懒加载提升性能
- **表单验证**：前端必填字段校验，后端二次验证
- **错误处理**：网络错误和操作失败都有明确提示
- **权限控制**：新增、编辑、删除按钮按权限显示

### 2.5 视觉规范

实际实现的界面特点：
- 列表展示：索引 + 排序输入框 + 缩略图 + 标题 + 描述 + URL + 时间 + 操作
- 表格特性：边框、斑马纹、加载状态、空状态处理
- 响应式：固定操作列，内容列最小宽度限制
- 交互反馈：悬停效果、按钮状态、消息提示
...
| 维度 | 验收标准 | 测试方法 | 状态 |
| --- | --- | --- | --- |
| **功能完整性** | ✅ 轮播图CRUD操作正常<br>✅ 排序功能实时生效<br>✅ 权限控制按角色正确<br>✅ 图片预览和上传正常 | 各模块功能测试<br>权限切换验证<br>文件上传测试 | ✅ 已实现 |
| **数据完整性** | ✅ 所有字段正确存储<br>✅ 图片URL格式验证<br>✅ 必填字段前端校验<br>✅ 分页和排序正确 | 数据验证测试<br>表单提交测试<br>API接口测试 | ✅ 已实现 |
| **用户体验** | ✅ 操作反馈及时明确<br>✅ 加载状态清晰展示<br>✅ 错误处理友好提示<br>✅ 表格交互流畅自然 | UI/UX测试<br>交互流程测试<br>性能测试 | ✅ 已实现 |
| **安全性** | ✅ 权限验证严格执行<br>✅ 文件上传安全限制<br>✅ XSS防护机制有效<br>✅ 操作日志完整记录 | 安全测试<br>权限测试<br>审计测试 | ✅ 已实现 |
| **兼容性** | ✅ 主流浏览器支持<br>✅ 响应式布局正常<br>✅ 图片格式兼容性好<br>✅ 网络异常处理完善 | 兼容性测试<br>响应式测试<br>异常测试 | ✅ 已实现 |
...
- **轮播图管理**：完整的CRUD操作，支持排序和图片预览
- **权限控制**：基于角色的操作权限精确控制
- **图片处理**：上传、预览、URL存储完整流程
- **数据验证**：前端校验与后端验证双重保障
- **用户体验**：加载状态、错误处理、操作反馈完善
...
- **基础CRUD**：100% ✅
- **图片管理**：90% ⚠️（支持预览和上传，缺少批量操作）
- **排序功能**：80% ⚠️（支持手动排序，缺少拖拽）
- **权限控制**：100% ✅
- **数据验证**：100% ✅

> **建议**：当前配置中心功能已经很完善，建议优先实现拖拽排序功能，提升用户操作体验。
