...
public class CourseControllerTest extends AbstractControllerTest {
    ...
    void shouldReturnPaginatedCourses_WhenValidParamsProvided() throws Exception {
        ...
        List<CourseDTO> courses = Arrays.asList(
            CourseDTO.builder().id(1L).name("瑜伽入门").price(BigDecimal.valueOf(99.00)).build(),
            CourseDTO.builder().id(2L).name("力量训练").price(BigDecimal.valueOf(129.00)).build()
        );
        ...
        mockMvc.perform(get("/api/courses")
               ...
               .andExpect(jsonPath("$.list[0].name").value("瑜伽入门"))
               .andExpect(jsonPath("$.list[1].name").value("力量训练"));
    }
}
```
...
class CourseBookingServiceTest {
    ...
    void shouldThrowException_WhenInsufficientBalance() {
        ...
        UserEntity user = UserEntity.builder()
            .id(userId)
            .balance(BigDecimal.valueOf(50.00))  // 余额不足
            .build();
...
class UserDaoTest extends AbstractDaoTest {
    ...
    void shouldUpdateUserSuccessfully() {
        ...
        assertThat(updated.getRealName()).isEqualTo("张三");
...
class JwtTokenServiceTest {
    ...
    void shouldReturnFalse_WhenTokenIsExpired() {
        // Given - 创建一个已过期的token（通过设置过去的时间）
        // 注意：实际实现中可能需要mock时间或使用特殊的过期token
...
(4, '舞蹈基础', '现代舞基础课程', 89.00, 45, 25, 3, NOW()),
(5, '游泳入门', '游泳基础课程', 79.00, 50, 12, 3, NOW());
