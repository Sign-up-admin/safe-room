...
class SecureStorage {
  ...
  set(key: string, value: string): void {
    if (typeof sessionStorage === 'undefined') {
      console.error('SecureStorage set error: sessionStorage is not available')
      return
    }
    try {
  ...
  get(key: string): string | null {
    if (typeof sessionStorage === 'undefined') {
      console.error('SecureStorage get error: sessionStorage is not available')
      return null
    }
    try {
  ...
  remove(key: string): void {
    if (typeof sessionStorage === 'undefined') {
      console.error('SecureStorage remove error: sessionStorage is not available')
      return
    }
    try {
  ...
  clear(): void {
    if (typeof sessionStorage === 'undefined') {
      console.error('SecureStorage clear error: sessionStorage is not available')
      return
    }
    try {
      // 由于sessionStorage API的限制，我们使用一个更简单的方法
      // 遍历所有可能的键（这不是最优的，但对于测试环境足够了）
