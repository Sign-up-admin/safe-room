# 系统管理需求文档（Admin System Management v1.0）

> 版本：v1.0  
> 更新日期：2025-11-15（计划日期）  
> 适用路由：`/users`（管理员账号）、操作日志（待实现）、权限管理（待实现）  
...
记录所有关键操作，包括数据新增、编辑、删除、导出等，支持审计和追溯。基于ModuleCrudPage组件实现。
...
| `type` | String | 操作类型（Add/Edit/Delete/Export/Login/Logout） | 是 |
...
| 查看详情 | 查看操作详情JSON | 复用ModuleCrudPage详情功能 | ✅ |
| 导出Excel | 导出筛选后的操作日志 | `GET /operationLog/export` | ✅ |
| 时间范围筛选 | 支持选择开始和结束时间 | 前端日期选择器 | ✅ |
| 操作类型筛选 | 下拉选择操作类型 | Add/Edit/Delete/Export/Login/Logout | ✅ |
...
export const MENU_CONFIG: MenuRole[] = [
  {
    ...
    backMenu: [                 // 后台菜单
      {
        child: [
          {
            allButtons: ['Add', 'View', 'Edit', 'Delete', 'Expiration Reminder'],
    ...
    hasBackLogin: 'yes',        // 是否支持后台登录
    tableName: 'users',         // 登录验证表名
  }
]
```
...
| `Home Total` | 首页统计 | 在首页显示该模块的统计数据 | ✅ |
| `Expiration Reminder` | 到期提醒 | 显示到期提醒功能 | ✅ |
| `Private Coaching Reservation` | 私教预约 | 允许预约私人教练 | ✅ |
| `Course Reservation` | 课程预约 | 允许预约课程 | ✅ |
| `Purchase` | 购买 | 允许购买会员卡等 | ✅ |
| `Membership Renewal Statistics` | 续费统计 | 显示续费统计数据 | ✅ |
| `Membership Purchase Statistics` | 购买统计 | 显示购买统计数据 | ✅ |
| `Course Daily Revenue` | 课程日收入 | 显示课程日收入统计 | ✅ |
| `Home Statistics` | 首页统计 | 在首页显示统计图表 | ✅ |
| `Refund` | 退款 | 允许处理退款 | ✅ |
| `View Comments` | 查看评论 | 允许查看课程评论 | ✅ |
| `Course Type Statistics` | 课程类型统计 | 显示课程类型统计 | ✅ |
| `Coach Course Statistics` | 教练课程统计 | 显示教练课程统计 | ✅ |
...
| 维度 | 标准 | 实现状态 |
| --- | --- | --- |
| 功能完整性 | 账号管理、操作日志查看功能正常 | ✅ |
| 权限控制 | 基于menu.ts的权限检查正确，无越权操作 | ✅ |
| 操作审计 | 关键操作正确记录到operation_log表 | ✅ |
| 安全性 | 密码字段隐藏、账号状态管理正常 | ✅ |
| 用户体验 | 操作流畅，错误提示清晰，表单验证完善 | ✅ |
| 数据完整性 | 所有必填字段验证，数据类型正确 | ✅ |
| 导出功能 | 操作日志支持Excel导出 | ✅ |
