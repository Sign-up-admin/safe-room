    ...
    mergeData() {
      ...
      if (this.events.length > 0 && this.timetable.length > 0) {
        for (let i = 0; i < this.events.length; i++) {
          // 获取星期几
          const week = this.weeks[this.events[i].week - 1]
          this.timetable[this.events[i].start - 1][week] = this.events[i]
        }
      }
    },
    objectSpanMethod({ row, column: _column, rowIndex, columnIndex }) {
      ...
      if (columnIndex === 0) {
        ...
        } else if (rowIndex > this.morningLength - 1 && rowIndex < this.morningLength + this.afternoonLength) {
          if (rowIndex === this.morningLength) {
            return {
