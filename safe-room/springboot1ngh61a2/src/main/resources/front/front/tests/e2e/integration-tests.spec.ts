...
test.describe('Cross-System Integration Tests', () => {
  test('should synchronize user data between front and admin systems', async ({ page, context }) => {
    ...
    await adminPage.goto('/#/login') // 假设后台管理系统在同一域名�?    await adminPage.fill('input[data-testid*="username"]', 'admin')
    await adminPage.fill('input[type="password"]', 'admin123')
    await adminPage.click('button[data-testid*="login-submit"]')

    // 验证管理员登录成�?    await expect(adminPage).toHaveURL(/\/dashboard|\/admin/)

    // 导航到用户管理页�?    await adminPage.goto('/#/admin/users')
    ...
    // 在后台管理系统修改用户信�?    const userRow = adminPage.locator(`[data-testid*="user-item"]`).filter({ hasText: testUsername })
    ...
    // 验证前端用户数据已更�?    await page.goto('/#/login')
    await page.fill('input[data-testid*="username"]', testUsername)
    await page.fill('input[type="password"]', testPassword)
  ...
  test('should synchronize course booking between systems', async ({ page, context }) => {
    ...
    // 在后台管理系统验证预约记�?    const adminPage = await context.newPage()
    ...
    // 验证新预约出现在列表�?    await expect(adminPage.locator(`[data-testid*="booking-item"]`).filter({ hasText: 'testuser' })).toBeVisible()
    if (courseName) {
      await expect(adminPage.locator(`[data-testid*="booking-item"]`).filter({ hasText: courseName })).toBeVisible()
    }

    // 在后台确认预�?    const bookingItem = adminPage.locator(`[data-testid*="booking-item"]`).filter({ hasText: 'testuser' }).first()
    ...
    // 验证前端预约状态已更新
    await page.reload()
    await expect(page.locator('[data-testid*="booking-status"]')).toContainText('已确认'should synchronize membership purchase between systems', async ({ page, context }) => {
    // 前端用户购买会员�?    await page.goto('/#/login')
    await page.fill('input[data-testid*="username"]', 'testuser')
    await page.fill('input[type="password"]', 'password123')
    await page.click('button[data-testid*="login-submit"]')
    ...
    // 验证前端会员状�?    await page.goto('/#/profile')
    await expect(page.locator('[data-testid*="membership-status"]')).toContainText('活跃')

    // 在后台管理系统验证会员购买记�?    const adminPage = await context.newPage()
    await adminPage.goto('/#/login')
    await adminPage.fill('input[data-testid*="username"]', 'admin')
    await adminPage.fill('input[type="password"]', 'admin123')
    await adminPage.click('button[data-testid*="login-submit"]')
    ...
    // 验证收入统计已更�?    const revenueStats = adminPage.locator('[data-testid*="total-revenue"]')
  ...
  test('should handle real-time notifications between systems', async ({ page, context }) => {
    ...
    // 后台管理员登�?    const adminPage = await context.newPage()
    await adminPage.goto('/#/login')
    await adminPage.fill('input[data-testid*="username"]', 'admin')
    await adminPage.fill('input[type="password"]', 'admin123')
    await adminPage.click('button[data-testid*="login-submit"]')
  ...
  test('should handle concurrent operations across systems', async ({ page, context }) => {
    ...
    // 所有用户同时尝试预约同一课程的同一时间�?    const bookingPromises = pages.map(async (userPage, index) => {
    ...
    // 等待所有预约操作完�?    const results = await Promise.allSettled(bookingPromises)
    ...
    // 验证后台管理系统显示正确的预约状�?    const adminPage = await context.newPage()
    await adminPage.goto('/#/login')
    await adminPage.fill('input[data-testid*="username"]', 'admin')
    await adminPage.fill('input[type="password"]', 'admin123')
    await adminPage.click('button[data-testid*="login-submit"]')
    ...
    // 验证后台数据一致�?    expect(confirmedBookings + pendingBookings).toBeGreaterThanOrEqual(1)
