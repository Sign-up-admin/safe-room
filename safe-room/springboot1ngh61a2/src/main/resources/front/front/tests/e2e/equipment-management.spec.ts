...
test.describe('器材管理模块测试', () => {
  ...
  test.describe('器材列表页面', () => {
    ...
    test('应支持器材类型筛�?, async ({ page }) => {
      ...
      // 验证筛选结�?
      const equipmentCount = await equipmentPage.getEquipmentCount()
      logTestStep(`筛�?有氧器材"后显�?${equipmentCount} 个器材`)
    })

    test('应支持器材状态筛�?, async ({ page }) => {
      const equipmentPage = new EquipmentListPage(page)
      await equipmentPage.goto()
      await equipmentPage.expectLoaded()
      ...
      // 验证筛选结�?
      const equipmentCount = await equipmentPage.getEquipmentCount()
      logTestStep(`筛�?可用"状态后显示 ${equipmentCount} 个器材`)
    })
  })

  test.describe('器材详情页面', () => {
    test('应正确显示器材详�?, async ({ page }) => {
      // 先访问器材列�?
  ...
  test.describe('器材预约流程', () => {
    ...
    test('应检测预约时间冲�?, async ({ page }) => {
      ...
      // 验证冲突提示
      const conflictMessage = page.locator('text=时间冲突, text=已被预约, text=不可�? .conflict-message, .el-message--warning')
    ...
    test('应验证预约状态管�?, async ({ page }) => {
      ...
      // 验证预约状态显�?
      const statusElement = page.locator('text=已预�? text=预约成功, .booking-status')
  ...
  test.describe('器材数据完整�?, () => {
    test('应验证器材卡片包含必要信�?, async ({ page }) => {
      const equipmentPage = new EquipmentListPage(page)
      await equipmentPage.goto()
      await equipmentPage.expectLoaded()
      ...
      logTestStep('验证器材卡片信息完整�?)
    })

    test('应验证器材详情页信息完整�?, async ({ page }) => {
      const equipmentListPage = new EquipmentListPage(page)
      await equipmentListPage.goto()
      await equipmentListPage.clickEquipmentCard(0)
      ...
      logTestStep('验证器材详情页信息完整�?)
    })
  })

  test.describe('响应式设计测�?, () => {
    test('应在移动端正确显�?, async ({ page }) => {
      // 设置移动端视�?
      ...
      logTestStep('验证移动端器材列表显�?)
    })

    test('应在平板端正确显�?, async ({ page }) => {
      // 设置平板端视�?
