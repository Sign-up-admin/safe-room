...
test.describe('课程管理模块测试', () => {
  ...
  test.describe('课程列表页面', () => {
    test('应正确显示课程列表'应支持课程搜索功�?, async ({ page }) => {
      ...
      logTestStep(`搜索"瑜伽"后显�?${courseCount} 个课程`)
    })

    test('应支持课程分类筛�?, async ({ page }) => {
      const coursePage = new CourseListPage(page)
      await coursePage.goto()
      await coursePage.expectLoaded()
      ...
      // 验证筛选结�?
      const courseCount = await coursePage.getCourseCount()
      logTestStep(`筛�?热门课程"后显�?${courseCount} 个课程`)
    })
  })

  test.describe('课程详情页面', () => {
    test('应正确显示课程详�?, async ({ page }) => {
      // 先访问课程列�?
  ...
  test.describe('课程预约流程', () => {
    ...
    test('应检测预约时间冲�?, async ({ page }) => {
      ...
      // 验证冲突提示
      const conflictMessage = page.locator('text=时间冲突, text=已有预约, text=不可�? .conflict-message, .el-message--warning')
  ...
  test.describe('课程数据完整�?, () => {
    test('应验证课程卡片包含必要信�?, async ({ page }) => {
      const coursePage = new CourseListPage(page)
      await coursePage.goto()
      await coursePage.expectLoaded()
      ...
      logTestStep('验证课程卡片信息完整�?)
    })

    test('应验证课程详情页信息完整�?, async ({ page }) => {
      const courseListPage = new CourseListPage(page)
      await courseListPage.goto()
      await courseListPage.clickCourseCard(0)
