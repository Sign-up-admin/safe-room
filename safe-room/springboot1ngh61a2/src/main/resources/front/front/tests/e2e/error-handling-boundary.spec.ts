...
test.describe('Error Handling and Boundary Conditions', () => {
  ...
  test.describe('网络错误处理', () => {
    test('应优雅处理网络连接失败'/#/courses')
      await page.waitForLoadState('domcontentloaded')

      // 验证离线状态处�?      await expect(page.locator('text=网络连接失败, text=网络异常, .network-error')).toBeVisible({ timeout: 5000 })
    ...
    test('应处理服务器错误响应', async ({ page }) => {
      ...
      await page.route('**/jianshenkecheng/**', async (route) => {
        await route.fulfill({
          status: 500,
          contentType: 'application/json',
          body: JSON.stringify({ code: 500, msg: '服务器内部错�? })
        })
      })
      ...
      // 验证服务器错误处�?      await expect(page.locator('text=服务器错�? text=系统异常, .server-error')).toBeVisible()
      logTestStep('服务器错误处理正�?)
    })
  })
  ...
  test.describe('表单验证边界情况', () => {
    test('应验证必填字段为空的情况', async ({ page }) => {
      ...
      // 直接提交空表�?      await page.click('button[type="submit"], button:has-text("注册")')
      ...
      logTestStep(`必填字段验证正常，发�?${errorCount} 个验证错误`)
    })

    test('应验证输入长度限�?, async ({ page }) => {
      await page.goto('/#/register')

      // 测试用户名超�?      const longUsername = 'a'.repeat(51) // 超过50字符限制
      await page.fill('input[name="yonghuzhanghao"]', longUsername)
      await page.click('button[type="submit"]')

      // 验证长度错误提示
      await expect(page.locator('text=长度不能超过, text=用户名过�? .length-error')).toBeVisible()
      logTestStep('输入长度限制验证正常')
    })

    test('应验证特殊字符输�?, async ({ page }) => {
      await page.goto('/#/register')
    ...
    test('应验证邮箱格�?, async ({ page }) => {
      ...
      for (const email of invalidEmails) {
        ...
        await expect(page.locator('text=邮箱格式错误, text=无效的邮�? .email-error')).toBeVisible()
    ...
    test('应验证手机号格式', async ({ page }) => {
      ...
      // 测试无效手机号格�?      const invalidPhones = [
        '1234567890', // 太短
        '123456789012', // 太长
        'abcdefghijk', // 非数�?        '1380013800a' // 包含字母
      ]
      ...
      for (const phone of invalidPhones) {
        ...
        await expect(page.locator('text=手机号格式错�? text=无效的手机号, .phone-error')).toBeVisible()
        logTestStep(`手机号格式验证正�? ${phone}`)
      }
    })

    test('应验证密码强�?, async ({ page }) => {
      await page.goto('/#/register')

      // 测试弱密�?      const weakPasswords = [
        '123', // 太短
        'password', // 无数�?        '123456789', // 无字�?        'PASSWORD', // 无小写字�?        'password' // 无特殊字符和数字
      ]
      ...
      for (const password of weakPasswords) {
        ...
        await expect(page.locator('text=密码强度不足, text=密码不符合要�? .password-error')).toBeVisible()
        logTestStep(`密码强度验证正常: ${password}`)
      }
    })

    test('应验证密码确认匹�?, async ({ page }) => {
      await page.goto('/#/register')
      ...
      // 验证密码不匹配错�?      await expect(page.locator('text=密码不匹�? text=两次密码不一�? .password-mismatch')).toBeVisible()
      logTestStep('密码确认匹配验证正常')
    })
  })

  test.describe('业务逻辑边界情况', () => {
    test('应处理重复预约冲�?, async ({ page }) => {
      // 登录用户
      ...
      if (await bookButton.isVisible()) {
        ...
        // 选择时间并提�?        const timeSlot = page.locator('.time-slot').first()
    ...
    test('应处理库存不足情�?, async ({ page }) => {
      ...
      // 尝试购买会员�?      await page.goto('/#/membership')
      ...
      if (await buyButton.isVisible()) {
        ...
        // 验证库存不足提示
        await expect(page.locator('text=已售�? text=库存不足, .out-of-stock')).toBeVisible()
  ...
  test.describe('用户界面异常情况', () => {
    test('应处理页面加载失�?, async ({ page }) => {
      ...
      // 页面应该仍然可用，只是样式丢�?      await expect(page.locator('h1, .page-title, text=健身')).toBeVisible()
      logTestStep('页面样式加载失败处理正常')
    })

    test('应处理图片加载失�?, async ({ page }) => {
      // Mock 图片加载失败
      ...
      // 验证图片错误处理（显示占位符或alt文本�?      const brokenImages = page.locator('img[src=""], img[alt], .image-placeholder')
    ...
    test('应处理JavaScript执行错误', async ({ page }) => {
      ...
      // 页面应该仍然可用（错误被处理�?      await expect(page.locator('body')).toBeVisible()
    ...
    test('应处理模态框意外关闭', async ({ page }) => {
      ...
      // 打开一个模态框（如果有�?      const modalTrigger = page.locator('button[data-modal], .modal-trigger').first()
      if (await modalTrigger.isVisible()) {
        ...
        // 模拟ESC键关�?        await page.keyboard.press('Escape')
  ...
  test.describe('数据异常处理', () => {
    test('应处理空数据列表', async ({ page }) => {
      // Mock 空数据响�?      await page.route('**/jianshenkecheng/**', async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
      ...
      // 验证空状态显�?      await expect(page.locator('.empty-state, text=暂无数据, text=没有找到')).toBeVisible()
      logTestStep('空数据列表处理正�?)
    })

    test('应处理数据格式错�?, async ({ page }) => {
      // Mock 格式错误的响�?      await page.route('**/jianshenkecheng/**', async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            code: 0,
            data: null, // 错误的格�?            total: 'invalid' // 错误的类�?          })
        })
      })
    ...
    test('应处理大数据量加�?, async ({ page }) => {
      ...
      expect(itemCount).toBeGreaterThan(10) // 至少显示一些项�?      logTestStep(`大数据量处理正常，显�?${itemCount} 个项目`)
    })
  })

  test.describe('并发操作处理', () => {
    test('应处理快速连续点�?, async ({ page }) => {
      await page.goto('/#/register')

      // 快速连续点击提交按�?      const submitButton = page.locator('button[type="submit"]').first()
      ...
      // 应该只有一个加载状�?      expect(loadingCount).toBeLessThanOrEqual(1)
      logTestStep('快速连续点击处理正�?)
    })

    test('应处理多标签页操�?, async ({ context }) => {
      // 在同一个上下文中打开多个页面
      const page1 = await context.newPage()
      const page2 = await context.newPage()

      // 在第一个页面登�?      await page1.goto('/#/login')
      ...
      // 在第二个页面也尝试登�?      await page2.goto('/#/login')
  ...
  test.describe('浏览器兼容性边�?, () => {
    test('应处理不同浏览器特�?, async ({ page }) => {
      // 测试localStorage可用�?      await page.addInitScript(() => {
      ...
      logTestStep('浏览器特性兼容性处理正�?)
    })

    test('应处理不同屏幕尺�?, async ({ page }) => {
      const viewports = [
