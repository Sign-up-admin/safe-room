...
test.describe('Front 用户注册到登录完整流程'设置测试环境完成')
  })

  test('新用户注册完整流�?, async ({ page }) => {
    logStep('开始新用户注册测试')
    ...
    await page.fill('input[name="shoujihaoma"], input[placeholder*="手机�?]', testUser.phone)
    ...
    if (await captchaInput.count() > 0) {
      await mockCaptcha(page) // 确保验证码Mock已设�?
      await captchaInput.fill('1234') // 测试验证�?
    }
  ...
  test('已注册用户登录流�?, async ({ page }) => {
    ...
    await page.getByPlaceholder('请输入会员账�?).fill(testUser.username)
    ...
    try {
      ...
      // 检查登录失败消�?
      const errorMessage = page.locator('.error-message, .el-message--error, text=账号或密码错�?)
      if (await errorMessage.count() > 0) {
        const errorText = await errorMessage.textContent()
  ...
  test.skip('忘记密码流程 - 功能未实�?, async ({ page }) => {
    // 跳过此测试，因为当前登录页面没有忘记密码功能
    logStep('忘记密码功能暂未实现，跳过测�?)
  })

  test('第三方登录流�?, async ({ page }) => {
    logStep('开始第三方登录测试')
  ...
  test('表单验证测试', async ({ page }) => {
    ...
    logStep(`表单验证工作正常，发�?${errorCount} 个错误提示`)
  ...
  test('网络延迟场景测试', async ({ page }) => {
    ...
    await page.selectOption('select[name="xingbie"]', '�?)
