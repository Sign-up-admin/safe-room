...
test.describe('增强测试功能演示', () => {
  test('智能重试和错误分类演示'智能重试演示', {
      category: 'error-handling',
      features: ['retry', 'error-classification']
    })

    await globalTestMonitor.recordStep('智能重试演示', '开始测�?, 'start')

    try {
      // 使用智能重试执行可能失败的操�?      await withSmartRetry(
        async () => {
        ...
        },
        {
          ...
          retryCondition: (error) => {
            ...
            // 只对元素未找到错误重�?            return categorized.type === 'element_not_found'
          }
        },
        '登录页面加载'
      )
      ...
      // 使用截图处理器捕获错误截�?      await screenshotHandler.withScreenshot(
        async () => {
          // 这里可以添加一些清理操�?          console.log('错误已通过截图记录')
        },
        '智能重试演示',
        {
  ...
  test('性能监控和截图演�?, async ({ page }) => {
    ...
    await globalTestMonitor.recordStep('性能监控演示', '开始页面加�?, 'start')
    ...
    try {
      ...
      // 条件截图 - 如果加载时间超过阈�?      const screenshotPath = await screenshotHandler.conditionalScreenshot(
        () => Promise.resolve(loadTime > 3000), // 如果加载超过3�?        'slow_page_load',
        { fullPage: true }
      )

      if (screenshotPath) {
        globalTestMonitor.recordScreenshot('性能监控演示', screenshotPath, '慢加载截�?)
      }
  ...
  test('完整的用户旅程监�?, async ({ page }) => {
    ...
    try {
      ...
      // 步骤2: 导航到功能页�?      await globalTestMonitor.recordStep(testName, '导航步骤', 'start')
      ...
      // 查找并点击课�?      const courseCards = page.locator('.course-card').first()
      if (await courseCards.isVisible()) {
        await courseCards.click()
        await waitForPageFullyLoaded(page)

        // 记录操作成功的截�?        const screenshotPath = await takeScreenshotWithTimestamp(page, 'course_selected')
        ...
        await globalTestMonitor.recordStep(testName, '无课程数�?, 'success', { note: '测试数据不足' })
      }
  ...
  // 测试后的报告导出
  test.afterAll(async () => {
    // 导出增强的测试报�?    const report = await globalTestMonitor.exportReport('./test-results/demo-enhanced-report.json')
    ...
    console.log(`错误类型: ${report.errors.errorTypes.join(', ') || '�?}`)
