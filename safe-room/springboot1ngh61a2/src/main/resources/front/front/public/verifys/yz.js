;(function(undefined) {
    ...
    RotateVerify.prototype = {
        ...
        _touchstart:function(){
            const _this = this;
			_this.$slideDragBtn.on('touchstart',(e) =>{
                _this.$slideDragBtn.css('pointer-events','none');
                setTimeout(() =>{_this.$slideDragBtn.css('pointer-events','all')},400)
        ...
        _touchmove:function(){
            const _this = this;
            _this.$slideDragBtn.on('touchmove',(e) =>{
                e.preventDefault();
                if(_this.dragTimerState || ifAnimate(_this.$slideDragBtn)){
                    return false;
                }else{
                    let x = e.originalEvent.targetTouches[0].pageX - _this.touchX;
					if(x<0){
					    x=0;
					}else if(x >=(_this.$slideDragWrap.width()-_this.$slideDragBtn.outerWidth())){
						x = _this.$slideDragWrap.width()-_this.$slideDragBtn.outerWidth();
        ...
        _touchend:function(){
            const _this = this;
            _this.$slideDragBtn.on('touchend',() =>{
